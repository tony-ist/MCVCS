 ...
 * {@code /vcs confirmPlace} then places the copy where it was left, and {@code /vcs cancelPlace} drops it.
 */
...
public class VcsPlacePreviewGameTest extends VcsGameTest {
	...
	protected void run(ClientGameTestContext context) {
		...
		try (TestSingleplayerContext singleplayer = context.worldBuilder().adjustSettings(settings -> settings.setAllowCommands(true)).create()) {
			...
			teleport(singleplayer, new BlockPos(shown.min().getX() + 1, shown.min().getY(), shown.min().getZ() - 6));
			context.waitTicks(2);
			...
			// Sprint held, a press moves ten blocks instead of one.
			box = sprintPress(context, PlacePreviewKeys.AWAY, box, 0, 0, PlacePreviewKeys.SPRINT_STEP);
			box = sprintPress(context, PlacePreviewKeys.CLOSER, box, 0, 0, -PlacePreviewKeys.SPRINT_STEP);
			...
			// Confirming puts the copy where it was left, gold corner and all, and stops showing it.
			BuildBox placed = box;
	...
	private static void checkKeyBindsScreen(ClientGameTestContext context) {
		context.runOnClient(client -> {
			for (KeyMapping key : new KeyMapping[] {PlacePreviewKeys.AWAY, PlacePreviewKeys.CLOSER, PlacePreviewKeys.LEFT,
				PlacePreviewKeys.RIGHT, PlacePreviewKeys.UP, PlacePreviewKeys.DOWN}) {
	...
	/** The same with the sprint key held, which is what makes a press move {@link PlacePreviewKeys#SPRINT_STEP} blocks. */
