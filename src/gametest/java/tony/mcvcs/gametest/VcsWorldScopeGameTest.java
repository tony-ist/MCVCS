...
/**
 * The {@code mcvcs/} folder is shared by every world in the game directory, but each project records the world it
...
public class VcsWorldScopeGameTest implements FabricClientGameTest {
	...
	public void runTest(ClientGameTestContext context) {
		...
		try (TestSingleplayerContext second = context.worldBuilder().adjustSettings(settings -> settings.setAllowCommands(true)).create()) {
			...
			List<String> names = second.getServer().computeOnServer(server -> ProjectRegistry.names(server));
			if (!names.isEmpty()) {
				throw new AssertionError("Expected no projects in a different world but got " + names);
			}
