...
/**
 * Draws the bounding box of the player's selected placement as a thin line outline.
 * <p>
 ...
 * gone when the player looks away or moves behind a wall. While a version is previewed there the outline
...
public final class SelectionBoxRenderer {
	/** Line color (ARGB): a light cyan that stands out against both terrain and sky; the selected placement's label shares it. */
	public static final int COLOR = 0xFF4FD8FF;
	/** Line color (ARGB) while the build is previewed: a light green; the previewed placement's label shares it. */
	...
	private static void emit(LevelExtractionContext context) {
		ClientPlacement placement = ClientPlacements.selected();
		if (placement == null || !placement.dimension().equals(context.level().dimension())) {
			return;
		}
		...
		Gizmos.cuboid(aabb, preview != null && preview.isOf(placement) ? PREVIEW_STYLE : STYLE);
	}
}
