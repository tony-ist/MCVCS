...
/**
 * {@code /vcs load [version]}: puts that version's schematic of the selected placement's build, or the latest one if
...
public final class VcsCommandLoad {
	...
	static int run(CommandSourceStack source, int version) throws CommandSyntaxException {
		...
		int loaded = version == VcsCommand.LATEST ? build.version() : version;
		...
		try {
			...
			source.sendSuccess(() -> Component.literal("Loaded build ").append(VcsMessages.name(build.name())).append(" v" + loaded
